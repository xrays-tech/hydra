...
const API_DOCS = [
  { tag: "System", endpoints: [
    ...
    { method: "POST", path: "/api/v1/reload", summary: "Hot-reload config from the DB",
      desc: "Rebuilds the in-memory config snapshot from SQLite (and re-resolves TLS certs). Returns the loaded snapshot counts plus whether this reload CHANGED the replicated content (`changed`) and the resulting `version`. A reload whose replicated content is unchanged is a no-op: the version advances only when the replicated bytes did, so replicas are not rebuilt for nothing. Pass `?force=1` to advance the generation unconditionally (ops override; parsed as a real query parameter). A fatal validation error keeps the old snapshot and returns 400.",
      auth: true,
      body: {},
      resp: ["200 — {\"status\":\"reloaded\",\"changed\":true,\"version\":7,\"tenants\":1,\"providers\":2,\"models\":3,\"keys\":2,\"certs\":0}",
             "200 (no-op) — {\"status\":\"reloaded\",\"changed\":false,\"version\":7,\"tenants\":1,\"providers\":2,\"models\":3,\"keys\":2,\"certs\":0}"],
    ...
    { method: "DELETE", path: "/api/v1/auth/cache", summary: "Invalidate the auth cache",
      desc: "Force re-authentication. Body is optional: {} (or empty body) clears EVERY tenant's cache; tenant_id clears one tenant; api_keys clears specific keys (across all tenants when tenant_id is omitted). In cluster mode the invalidation is broadcast and the operator WAITS for the fleet to confirm it: the `fleet` object reports `applied` (200), `pending` (202, with the lagging nodes named) or `single_node` (200, this node is the whole fleet). There is no `published` boolean any more — it could not be false when there was no stream, so it asserted a broadcast that never happened. Tenant self-service equivalents live on the DATA plane under /tenant/{tenant_id}/api/v1/.",
      auth: true,
      body: { tenant_id: "acme", api_keys: ["sk_abc123"] },
      resp: ["200 — {\"invalidated\":1,\"checked\":1,\"tenant_id\":\"acme\",\"scope\":\"keys\",\"fleet\":{\"state\":\"applied\",\"nodes_total\":3,\"nodes_applied\":3,\"lagging\":[],\"event_id\":\"1737-0\",\"waited_ms\":41}}",
             "202 — same body with \"state\":\"pending\" and the unconfirmed nodes listed in \"lagging\""],
      errors: [
        { status: 400, code: "invalid_json", desc: "request body is not valid JSON" },
        { status: 400, code: "too_many_keys", desc: "more than 1000 api_keys in one request" },
        { status: 400, code: "invalid_api_key", desc: "an api_key is longer than 4096 bytes" },
        { status: 503, code: "…", desc: "the invalidation channel exists but did not answer (fail-closed: the fleet was NOT told)" } ] },
