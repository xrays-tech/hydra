...
| 2026-09-16 | plan | dev-docs/aegis/plans/2026-09-16-changelog-gap-remediation.md | changelog 缺口对齐修复计划（P0+P1+P2；快照 wire v2 双向 fail-closed + 复制内容唯一所有者 + 注册表回收 + h2 authority + 会话持久化 + CI ui-e2e）—— **✅ 第 12 轮 oracle 复审 PASS（门禁通过）；累计 12 轮、阻塞项 45→0。** 开发进度：**Phase 0 ✅ / Phase A（Batch 1–4：T1+T6、T2、T3、T4）✅ / Phase B（Batch 5：T5、T8、T7）✅ / Phase C 部分（Batch 6：T9.1、T9.2、T9.4；T9.6 记录为非目标、T9.7 由 T1 交付）**。后实现对抗式 oracle 复审：T1+T6 PASS、T3 PASS、T4 PASS、T2 FAIL（2 阻塞已修：`ops.md` 假引用 + 回收会把活着的旧版本节点永久注销 ⇒ 改为两击规则）；被证伪的 6 条文档断言已全部改正。两次统一门禁均 `fail=0`（Phase B 含真实浏览器腿 Playwright **11 passed**）；最新全量：`--features server` 27 套件 0 failed、三特性 **422 passed / 0 failed**。**计划中仍待实施**：T9.3（租户写后置步骤事务化）、T9.5（非 leader 横幅）、Phase D（T10.1–T10.6）。 |
