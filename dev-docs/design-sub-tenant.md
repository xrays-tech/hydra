...
A-1（`design-tenant-api.md` §6.4）否决的是**失效清除端点（E2）**向 leader 转发，其决定性理由是“扇出已由共享流完成，转发不产生额外效果”。该理由对**配置写不成立**——leader 是唯一写者，转发就是唯一正确的落点；但 A-1 的机制偏好（“数据面经共享 Redis 总线协调、不直连 peer”）需要**显式修订**。

**A-2 已落（2026-09-18）**：按本项目决策记录归属（无 ADR 体系，见 `design-tenant-api.md` §6.5），A-2 写在 **`design-tenant-api.md` §6.4b「决策记录 A-2：租户配置写经 internal 控制面转发到 leader」**，状态**已决**，含 v1 复审 findings 1/2 的实现前置条件，并显式声明**仅限定修订配置写场景、A-1 对 E2 继续有效**。**不得悄悄绕过 A-1。**
