...
另外，按 ADR 门禁的 **Retro/Memory Filter**：本决策在本文写作时**尚未执行**（设计状态为待评审），属于"unexecuted idea"，**不应**进入被接受的架构记忆（即不应新建独立 ADR 文件）。这也是本记录采用"正文内决策记录"这种更轻形态的原因；实现落地后若需要，再按项目后续是否引入 ADR 体系决定是否升格。**本决策**同时**已**被 §11 收口为"已决项"，不在待决策清单内。
...
| 风险 | 级别 | 缓解 | 回滚 |
|---|---|---|---|
| 租户令牌在公网端口被爆破 | 中 | 快照校验（去掉 DB 放大）+ 双维度限流 + 锁定 | 总开关 `HYDRA_TENANT_API=off`；或只在可信网络开放租户 API |
